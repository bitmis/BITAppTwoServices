...
public interface LateralApplicantsPersonlInfoRepository extends JpaRepository<LateralApplicantsPersonlInfo, String> {
    ...
    @Transactional
    @Modifying
    @Query("update LateralApplicantsPersonlInfo l set l.full_name = ?1, l.full_name_sinhala = ?2, l.full_name_tamil = ?3, l.name_marking = ?4, l.initials = ?5, l.title = ?6, l.gender = ?7, l.id_type = ?8, l.id_no = ?9, l.dob = ?10, l.email = ?11 , l.citizenship = ?12 , l.nationality = ?13 , l.disabilities = ?14 where l.application_no = ?15")
    ...
    @Transactional
    @Modifying
    @Query("update LateralApplicantsPersonlInfo l set l.phone = ?1, l.mobile = ?2, l.address1 = ?3, l.address2 = ?4, l.address3 = ?5, l.district = ?6, l.country = ?7 where l.application_no = ?8")
